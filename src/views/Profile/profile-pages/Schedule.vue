<template>
  <div class="">
    <div class="title-wrapper">
      <h2>{{ this.$t('myTrainings') }}:</h2>
    </div>
    <div v-if="trainingsList?.length > 0" class="trainings-list__wrapper">
      <WeekTrainings class="profile_tlw" :isProfile="true" :daily-tr="trainingsList" @rescind="onRescind" :key="trainingsList"/>
    </div>
    <div v-else-if="trainingsList?.length <= 0" class="trainings-block animate__animated animate__fadeIn">
      <p>{{ this.$t('noGroupTrainings') }}</p>
    </div>
    <div v-else>
      <PreloaderSmall/>
    </div>
...
.trainings-block p:first-letter{
