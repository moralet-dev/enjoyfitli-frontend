<template>
  <div class="wrapper">
    <div class="content" v-if="req?.id && req?.status === 'declined' && me.id === req?.user">
      <p class="title">{{$t('paidFail')}}!</p>
      <div class="content">
        <p>{{$t('paymentData')}}:</p>
        <div class="detail">
      ...
      <p class="note">*{{$t('paymentFailNote')}}<br><a href="tel:+380737304509" class="text-link">+380 (73) 730 45 09</a></p>
      <div class="links">
        <router-link class="btn" :to="{name:'payment'}">{{ $t('payment') }}</router-link>
